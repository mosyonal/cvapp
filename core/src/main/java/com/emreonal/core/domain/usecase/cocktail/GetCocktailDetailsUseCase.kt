...
class GetCocktailDetailsUseCase @Inject constructor(private val repository: CocktailsRepository) {

    operator fun invoke(id: String?): Flow<Resource<List<Cocktail>>> = flow {
        emit(Resource.Loading())
        try {
            val response = repository.getCocktailsById(id)
